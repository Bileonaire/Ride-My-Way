...
class RequestRideTests(BaseTests):
    ...
    def test_admin_get_allrequests(self):
        ...
        response = self.app.get('/api/v1/requests', headers=self.admin_header)
        self.assertEqual(response.status_code, 200)

    def test_user_get_one(self):
        """Tests user successfully getting a request"""
        response = self.app.get('/api/v1/requests/2', headers=self.user_header)
        self.assertEqual(response.status_code, 200)

    def test_get_non_existing(self):
        """Test getting a request while providing non-existing id"""
        response = self.app.get('/api/v1/requests/50', headers=self.user_header)
    ...
    def test_good_deletion(self):
        ...
        response = self.app.delete('/api/v1/requests/1', headers=self.user_header)
        self.assertEqual(response.status_code, 200)
    
    def test_bad_deletion_not_your_request(self):
        """Test a unsuccessful request deletion"""
        response = self.app.delete('/api/v1/requests/1', headers=self.driver_header2)
        self.assertEqual(response.status_code, 404)

    def test_deleting_non_existing(self):
        """Test deleting request that does not exist"""
        response = self.app.delete('/api/v1/requests/500', headers=self.user_header)
        self.assertEqual(response.status_code, 404)
