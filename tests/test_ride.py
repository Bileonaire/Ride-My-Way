...
class RideTests(BaseTests):
    ...
    def test_admin_get_one(self):
        ...
        response = self.app.get('/api/v1/rides/1', headers=self.user_header)
        self.assertEqual(response.status_code, 200)

    def test_user_get_one(self):
        """Tests user successfully getting a ride"""
        response = self.app.get('/api/v1/rides/2', headers=self.user_header)
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_non_existing(self):
        ...
        response = self.app.get('/api/v1/rides/300', headers=self.user_header)
    ...
    def test_good_ride_update(self):
        ...
        added_ride = self.app.post( # pylint: disable=W0612
            '/api/v1/rides', data=initial_data,
    ...
    def test_good_deletion(self):
        ...
        response = self.app.delete('/api/v1/rides/1', headers=self.driver_header)
        self.assertEqual(response.status_code, 200)
    ...
    def test_deleting_non_existing(self):
        ...
        response = self.app.delete('/api/v1/rides/200', headers=self.driver_header)
        self.assertEqual(response.status_code, 404)
    
    def test_start_ride(self):
        """Test starting a ride successfully"""
        response = self.app.post('/api/v1/rides/1', headers=self.driver_header)
        self.assertEqual(response.status_code, 200)
    
    def test_unsuccessful_start_ride(self):
        """Test starting a ride unsuccessfully"""
        response = self.app.post('/api/v1/rides/5', headers=self.driver_header2)
        self.assertEqual(response.status_code, 404)
