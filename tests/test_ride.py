...
class RideTests(BaseTests):
    ...
    def test_user_get_one(self):
        ...
        response = self.app.get('/api/v3/rides/2', headers=self.user_header)
        self.assertEqual(response.status_code, 200)
    ...
    def test_unsuccess_user_deleting_ride(self):
        """Test a successful ride deletion"""
        response = self.app.delete('/api/v3/rides/3', headers=self.user_header)
    ...
    def test_start_ride(self):
        ...
        response = self.app.post('/api/v3/rides/2', headers=self.driver_header)
        self.assertEqual(response.status_code, 200)
