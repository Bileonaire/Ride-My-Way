...
class User(object):
    ...
    @staticmethod
    def create_user(username, email, password, usertype, carmodel=None, numberplate=None, **kwargs):
    ...
    def update_user(user_id, usertype, username, email, password, carmodel=None, numberplate=None):
        ...
        if user_id in all_users.keys():
            all_users[user_id] = {"id" : user_id, "username" : username,
                                 ...
                                 "carmodel" : carmodel, "numberplate" : numberplate, "usertype" : usertype}
    ...
    def all_users():
        ...
        for user in all_users:
            ...
            if user["usertype"] == "driver":
                profile = {user['id'] : { "Username" : user['username'], "Email" : user['email'], "Usertype" : user['usertype'],
                    "Car Model" : user["carmodel"], "Number Plate" : user["numberplate"], "Rides already given" : [] , 
                    "Rides pending" : [] }}
                ...
                profile = {user['id'] : { 'username' : user['username'], 'email' : user['email'], 'usertype' : user['usertype'],
                                        "Rides already taken" : [] , "Rides pending" : [] ,  "Rides rejected" : [] }}
                for request in all_requests:
                    ...
                    if request["user_id"] == user["id"]:
                        ...
                        if request["status"] == "taken":
                            profile[user['id']]["Rides already taken"].append(all_rides[request["ride_id"]]["trip"])
                        if request["status"] == "pending":
                            profile[user['id']]["Rides pending"].append(all_rides[request["ride_id"]]["trip"])
                        if request["status"] == "rejected":
                            profile[user['id']]["Rides rejected"].append(all_rides[request["ride_id"]]["trip"])
...
class Ride(object):
    ...
    @staticmethod
    def create_ride(departurepoint, destination, driverid, departuretime, cost, maximum, status="pending", **kwargs):
        ...
        all_rides[ride_count] = {"id": ride_count, "trip" : departurepoint + " to " + destination,
                                 ...
                                 "cost": cost, "maximum": maximum, "passengers" : [], "status" : status}
        new_ride = all_rides[ride_count]
        ride_count += 1
        return new_ride

    @staticmethod
    def update_ride(ride_id, departurepoint, destination, driverid, departuretime, cost, maximum, **kwargs):
        ...
        if ride_id in all_rides.keys():
            ...
            all_rides[ride_id] = {"id": ride_id, "trip" : departurepoint + " to " + destination,
                                 "driver_id": driverid, "departuretime": departuretime,
                                 "cost": cost, "maximum": maximum, "passengers": passengers, "status" : status}
            return all_rides[ride_id]
        return {"message" : "ride does not exist"}

    @staticmethod
    def start_ride(ride_id, driver_id, **kwargs):
...
class Requests(object):
    ...
    @staticmethod
    def request_ride(ride_id, user_id, accepted=False, status="pending", **kwargs):
    ...
    def update_request(request_id, **kwargs):
        ...
        if passengers < maximum:
            if all_requests[request_id]["accepted"] == False:
            ...
            elif all_requests[request_id]["accepted"] == True:
            ...
            if all_requests[request_id]["accepted"] == True:
                all_requests[request_id]["accepted"] = False
                accepted.remove(all_users[all_requests[request_id]["user_id"]]["username"])
                return {"message" : "the request has been rejected"}
